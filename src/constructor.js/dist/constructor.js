...
;
  ...
  */
  ...
  window.Constructor.showLayoutScheme = function() {
    ...
    s = void 0;
    recount = function() {
      return s.text("Ширина блока: " + _this._block_width(1) + "; Высота блока: " + _this._block_height(1));
    };
    $.each(lo, function(i, val) {
      var changer_1, inner, l, sp;
      changer_1 = function(event, ui) {
        _this.Site.layout[i] = ui.value;
        return recount();
      };
      if (i !== 'fixed') {
        ...
        if (i === "grid" || i === "padding") {
          inner = $("<ul>").appendTo(l);
          return $.each(val, function(j, val) {
            var changer_2, sp;
            changer_2 = function(event, ui) {
              _this.Site.layout[i][j] = ui.value;
              return recount();
            };
            l = $("<li>").text(labels[j]).appendTo(inner);
            sp = $("<input>").appendTo(l).spinner({
              spin: changer_2
            });
            sp.spinner("value", val);
            return sp.on('keyup', function(e) {
              var v;
              v = $(e.target).val();
              _this.Site.layout[i][j] = parseInt(v);
              return recount();
            });
          });
        } else {
          ...
          return sp.on('keyup', function(e) {
            var v;
            v = $(e.target).val();
            _this.Site.layout[i] = parseInt(v);
            return recount();
          });
        }
      }
    });
    s = $('<div></div>').text("Ширина блока: " + this._block_width(1) + "; Высота блока: " + this._block_height(1)).appendTo(to);
    ...
    $("<button>").text("cancel").click(function() {
      _this.Site.layout = lo;
      _this.showLayoutScheme();
      return _this.redraw();
    }).appendTo(finaldiv);
...
//					.text("Here's fonts scheme with Serifs in headers, Sans in texts").appendTo(to)
  ...
  window.Constructor._show_pattern_editor = function(to) {
    var control, pat_cont, preview, self, show_control,
      _this = this;
    self = this;
    to.find("*").remove();
    control = $("<div>").appendTo(to).width(400).height(400).css("float", "left");
    to.height(400);
    ...
    $.each(DB.get_objects("generic." + BASE_SITE, "pattern", {
      type: "constructor"
    }).objects, function(ix, obj) {
      var d;
      d = $("<div>").css("float", "left").css("background-size", "32px 32px").width(48).height(48).css("margin", "5px").appendTo(pat_cont).css("cursor", "pointer").on("mouseenter", function() {}).on("click", function() {
        var prev;
        preview.find("*").remove();
        prev = $("<div>").width("100%").height("100%").appendTo(preview);
        return _this.renderPattern(prev, obj);
      }).on("dblclick", function() {
        return show_control(obj);
      });
      return _this.renderPattern(d, obj);
    });
    show_control = function(pattern) {
      var A, BG, C, Cjq, FG, GA, S, Z, base, buffData, choose_bg, choose_fg, ctx, exc_color, flright, image, img_cont, img_src, li, library_ix, make_image, my_patt, opacity, prev, redraw_ctx, result, sctx, ul, _i;
      if (pattern == null) {
        pattern = false;
      }
      redraw_ctx = function() {
        ...
        ctx.restore();
        img = C.toDataURL();
        return prev.css("background", "url(" + img + ") repeat");
      };
      control.find("*").remove();
      preview.find("*").remove();
      ...
      if (pattern) {
        ...
        FG = pattern.FG;
        img_src = pattern.base_image;
        _i = $(new Image()).one('load', function(e) {
          result = img_src;
          exc_color(e.target);
          return redraw_ctx;
        }).prop('src', img_src);
      ...
      make_image = function() {
        ...
        img = new Image();
        img.onload = function() {
          return exc_color(img);
        };
        return img.src = result;
      };
      exc_color = function(img_) {
        var FGA, alphas, c, du, ix, x, y, _I;
        S.width = img_.width;
        S.height = img_.height;
        sctx.drawImage(img_, 0, 0);
        buffData = sctx.getImageData(0, 0, img_.width, img_.height);
      ...
      $.each(back_icons_urls, function(i, url) {
        return $(new Image()).one("load", function() {
          var img_self;
          img_self = this;
          return $("<div>").css("float", "left").width(64).height(64).css("border", "2px solid black").css("background", "url(" + this.src + ") no-repeat").css("background-size", "64px 64px").css("margin", "5px").appendTo(img_cont).on("mouseenter", function() {
            return $(this).css("border-color", "black");
          }).on("mouseleave", function() {
            return $(this).css("border-color", "grey");
          }).on("click", function() {
            result = url;
            return exc_color(img_self);
          });
        }).prop("src", url);
      });
      ...
      $("<button>").click(function(e) {
        return self.draw_color_chooser(choose_bg).appendTo(my_patt);
      }).appendTo(my_patt).text("Background");
      $("<button>").click(function(e) {
        return self.draw_color_chooser(choose_fg).appendTo(my_patt);
      ...
      return $("<div>").appendTo(control).append($("<button>").text("Сохранить").click(function() {
        var ready_image;
        ...
        DB.save_object("generic." + BASE_SITE, "pattern", pattern, (function() {}), (function() {
          _this.redraw_background();
          return _this.redraw();
        }));
        return self._show_pattern_editor(to);
      })).append($("<button>").text("Отменить").click(function() {
        return self._show_pattern_editor(to);
      }));
    };
    return $("<button>").appendTo(control).text("Новый").click(function() {
      control.find("*").remove();
      return show_control();
    });
  };
  ...
  window.Constructor.showSEOScheme = function() {
    var fv, li, meta_google, meta_yandex, save_metas, self, seod, sname, ul,
      _this = this;
    self = this;
    meta_yandex = void 0;
    meta_google = void 0;
    ...
    $("<input type=\"button\">").val("Запустить").button().appendTo(li).click(function() {
      $(this).hide();
      self.caching();
      return $(this).show();
    });
    ...
    if (this.Site.favicon != null) {
      $('<img>').prop('src', this.Site.favicon).appendTo(fv);
    }
    return $('<input>').attr('type', 'file').appendTo(li).change(function(e) {
      var files, fr;
      files = e.target.files;
      fr = new FileReader();
      fr.onload = function(e) {
        _this.Site.favicon = e.target.result;
        fv.find('*').remove();
        return $('<img>').prop('src', _this.Site.favicon).appendTo(fv);
      };
  ...
  window.Constructor.showDomainScheme = function() {
    var T, addToList, adder, dialog, dom_name, ul,
      _this = this;
    T = function(t) {
      return $('<' + t + '>');
    };
    ...
    ul = void 0;
    addToList = function(dom, i) {
      var li;
      li = T('li').appendTo(ul).text(dom);
      return T('button').text('X').appendTo(li).click(function() {
        _this.Site.domains.splice(i + 1, 1);
        li.remove();
        return _this._save_site();
      });
    };
    if (this.Site.domains == null) {
      this.Site.domains = [window.location.host];
    }
    ...
    T('b').appendTo(dialog).text(this.Site.domains[0]);
    if (this.Site.domains.length > 1) {
      T('h3').appendTo(dialog).text('Дополнительные домены');
      ul = T('ul').appendTo(dialog);
      _.chain(this.Site.domains.slice(1)).each(addToList);
    }
    ...
    T('input').prop('name', 'domain_name').val('Добавить домен').appendTo(adder).prop('type', 'button').click(function() {
      var dconf, key, mess, val;
      val = dom_name.val();
      ...
      T('button').appendTo(dconf).text('Подтвердить').click(function() {
        return $.ajax({
          url: '/_/check_domain/',
          data: {
            domain: val,
          ...
          failure: function() {
            return mess.text("Домен не подтвержден, Попробуйте еще!");
          },
          dataType: 'json',
          success: function(js) {
            if (js.domain_confirm) {
              _this.Site.domains.push(val);
              addToList(val, _this.Site.domains.length);
              _this._save_site();
              return dconf.dialog('close');
            } else {
              return mess.text("Домен не подтвержден, Попробуйте еще!");
            }
          }
  ...
  window.Constructor.show_CP = function(active_tab) {
    var C, E, R, SF, adder, app_menu, app_menu_template, d, k, pa, pages, self, template_create, template_search_results, template_searcher, ul, v,
      _this = this;
    self = this;
    C = $("div#controls");
    ...
    $("<li>").append($("<a>").prop("href", "#").text("SEO и favicon").click(function() {
    ...
    $.each(pa, function(ix, p) {
      ...
      li.append($("<button>").button({
        ...
        var descr, diag, kw, page_slug;
        page_slug = $("<input>").appendTo(li).val(i).keyup(function() {});
        diag = $("<div>").dialog({
          title: "Опции страницы",
          width: 600,
          height: 300,
...
<div>\
  ...
  <div id = "app_dialog">\
    ...
    template_searcher = "    <div id='id_search_dialog'>      <ul id='my_apps'>        {{# apps }}          <li> {{ title }} {{#in_app}}<span style='color:green;'>Уже добавлено</span> {{/in_app}} {{^in_app}} <input class='_add_button' type='button' app_name='{{ app_name }}' value='+'> {{/in_app}}</li>        {{/apps }}      </ul>      <input id='id_app_search_input' />      <div id='id_search_results'>      </div>    </div>  ";
    template_search_results = "  <ul>    {{#apps}}      <li> {{ title }}  {{#in_app}}<span style='color:green;'>Уже добавлено</span> {{/in_app}} {{^in_app}}<input class='_add_button' type='button' app_name='{{full_name}}' value='+'>{{/in_app}} </li>    {{/apps}}  </ul>  ";
    E = function(evt) {
      var app, html, source;
    ...
    R = function(evt) {
      var app, ix;
      app = $(evt.target).attr('app_name');
      if (app !== "generic." + BASE_SITE) {
        if (_this.Site.Applications[app].remove != null) {
          _this.Site.Applications[app].remove();
        }
        delete _this.Site.Applications[app];
        ix = _this.Site._Apps.indexOf(app);
        _this.Site._Apps.splice(ix, 1);
        _this._save_site();
        _this.redraw();
        return self.redraw_cp(2);
      }
    };
